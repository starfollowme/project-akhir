...
export default function AdminProductsPage() {
  ...
  return (
    <div className="p-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Produk ({pagination.total})</CardTitle>
        </CardHeader>
        <CardContent>
          {products.length > 0 ? (
            <>
                  ...
                  <tbody>
                    {products.map((product) => (
                      ...
                      >
                        ...
                        <td className="p-3">
                          <span className={`px-2 py-1 rounded-full text-xs ${product.stock > 10 ? 'bg-green-100 text-green-800' : product.stock > 0 ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800'}`}>{product.stock} unit</span>
                        </td>
                        <td className="p-3">
                          <Badge variant={product.isActive ? 'default' : 'secondary'}>{product.isActive ? 'Aktif' : 'Tidak Aktif'}</Badge>
                        </td>
                        <td className="p-3">
            ...
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 mb-4">{searchTerm || selectedCategory !== 'all' || statusFilter !== 'all' ? 'Coba sesuaikan kriteria pencarian atau filter Anda' : 'Mulai dengan menambahkan produk pertama Anda'}</p>
              <Link href="/admin/products/create">
                <Button>
                  <Plus className="w-4 h-4 mr-2" />
                  Tambah Produk
